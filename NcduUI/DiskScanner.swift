...
final class DiskScanner: @unchecked Sendable {
    ...
    private func readDirectory(
        ...
        while let entp = readdir(dir) {
            ...
            if !filters.excludePatterns.isEmpty, PathMatching.matchesExclude(childPath, patterns: filters.excludePatterns) {
            ...
            if filters.excludeCaches, PathMatching.hasCacheDirTag(childPath) {
                child.flags.insert(.excluded)
                child.ownSize = 0
                child.ownASize = 0
                continue
